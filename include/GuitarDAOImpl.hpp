...
protected:
    ...
    int32_t add_guitar_string(const GuitarStringRecord& guitar_string_record) const;

    std::vector<GuitarAdjustmentRecord> get_guitar_adjustments(int guitar_id) const;
    int32_t add_guitar_adjustment(const GuitarAdjustmentRecord& guitar_adjustment_record) const;

    std::vector<GuitarStringAdjustmentRecord> get_guitar_string_adjustments(int guitar_adjustment_id) const;
    int32_t add_guitar_string_adjustment(const GuitarStringAdjustmentRecord& guitar_string_adjustment_record) const;
};
