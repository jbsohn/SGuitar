...
impl Note {
    pub fn new_with_note_value(note_value: NoteValue, octave: i32) -> Self {
        Self {
            midi_value: ((octave + 1) * 12) + note_value as i32,
        }
    }

    pub fn new_with_midi_value(midi_value: i32) -> Self {
        Self { midi_value }
    }
...
mod tests {
    ...
    fn test_new_note_value() {
        let note = Note::new_with_note_value(NoteValue::C, 4);
    ...
    fn test_new_midi_value() {
        let note = Note::new_with_midi_value(60);
        assert_eq!(note.note_value(), NoteValue::C);
        assert_eq!(note.midi_value(), 60);
        assert_eq!(note.octave(), 4);
        assert_eq!(note.description(), "C-4")
