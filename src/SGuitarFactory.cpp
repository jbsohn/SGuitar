...
std::shared_ptr<Guitar> GuitarFactory::create_guitar(const GuitarRecord& guitar_record) {
...
std::shared_ptr<Scale> GuitarFactory::create_scale(const NoteValue root_note, const ScaleRecord& scale_record) {
    return Scale::create_with_root_note(root_note, scale_record.semitones);
}

std::shared_ptr<Chord> GuitarFactory::create_chord(const NoteValue root_note, const ChordRecord& chord_record) {
